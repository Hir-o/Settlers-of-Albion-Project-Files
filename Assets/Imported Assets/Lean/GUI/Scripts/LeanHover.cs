...
namespace Lean.Gui
{
	...
	public class LeanHover : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
	{
		...
		private CanvasGroup upgradeButtonCanvasGroup;
		...
		private void Start()
		{
			if (GetComponentInParent<CanvasGroup>()) upgradeButtonCanvasGroup = GetComponentInParent<CanvasGroup>();
		}

		public void OnPointerEnter(PointerEventData eventData)
		{
			if (upgradeButtonCanvasGroup != null)
				if (upgradeButtonCanvasGroup.alpha <= 0f) return;
		...
		{
			enteredPointers.Clear();
			
			if (_eventData.button == PointerEventData.InputButton.Left)
			{
				enteredPointers.Add(_eventData.pointerId);
		...
		public void ManualPointerExit()
		{
			if (_eventData != null)
			{
				if (enteredPointers.Remove(_eventData.pointerId) == true)
				{
					if (MultiExit == false && enteredPointers.Count > 0)
					{
						return;
