...
namespace TbsFramework.Players
{
    ...
    public class NaiveAiPlayer : Player
    {
        ...
        public NaiveAiPlayer() { _rnd = new System.Random(); }
        ...
        private IEnumerator Play()
        {
            ...
            foreach (var unit in myUnits.OrderByDescending(u => u.Cell.GetNeighbours(_cellGrid.Cells)
                                                                 .FindAll(u.IsCellTraversable).Count))
            {
                var enemyUnits   = _cellGrid.Units.Except(myUnits).ToList();
                var unitsInRange = new List<Unit>();
                foreach (var enemyUnit in enemyUnits)
                {
                    if (unit.IsUnitAttackable(enemyUnit, unit.Cell)) { unitsInRange.Add(enemyUnit); }
                ...
                if (unitsInRange.Count != 0)
                {
                    if (AITurnSpeedController.Instance != null && AITurnSpeedController.Instance.aiTurnSpeed ==
                        AITurnSpeedController.AITurnSpeed.Normal)
                    {
                        TBSCamera.Instance.FocusOn(unit.transform);
                ...
                foreach (var potentialDestination in potentialDestinations)
                {
                    ...
                    if ((shortestPath == null && path.Sum(h => h.MovementCost) > 0) || shortestPath != null &&
                        (path.Sum(h => h.MovementCost) < shortestPath.Sum(h => h.MovementCost) &&
                ...
                {
                    foreach (var potentialDestination in shortestPath
                                                         .Intersect(unit.GetAvailableDestinations(_cellGrid.Cells))
                                                         .OrderByDescending(h => h.GetDistance(unit.Cell)))
                    {
                        var path     = unit.FindPath(_cellGrid.Cells, potentialDestination);
                        var pathCost = path.Sum(h => h.MovementCost);
                        if (pathCost > 0 && pathCost <= unit.MovementPoints)
