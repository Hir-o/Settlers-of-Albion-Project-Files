...
namespace TbsFramework.Units
{
    ...
    public abstract class Unit : MonoBehaviour
    {
        ...
        public UnitType unitType = UnitType.Footman;

        [HideInInspector]
        public int InitMaxHitPoints; // store initial max hitpoints
        ...
        private Building _siegedBuilding;

        public virtual float MovementPoints { get { return movementPoints; } protected set { movementPoints = value; } }
        ...
        public virtual void Initialize()
        {
            ...
            InitMaxHitPoints    = MaxHitPoints;
            TotalHitPoints      = HitPoints;
            TotalMovementPoints = MovementPoints;
            TotalActionPoints   = ActionPoints;

            _siegedBuilding = null;
        ...
        public virtual void OnUnitClicked()
        {
            ...
            if (UnitClicked != null) { UnitClicked.Invoke(this, new EventArgs()); }
        }
        ...
        protected virtual void OnMouseEnter()
        {
            ...
            if (UnitHighlighted != null) { UnitHighlighted.Invoke(this, new EventArgs()); }
        }
        ...
        protected virtual void OnMouseExit()
        {
            ...
            if (UnitDehighlighted != null) { UnitDehighlighted.Invoke(this, new EventArgs()); }
        }
        ...
        protected virtual void OnDestroyed()
        {
            ...
            if (_siegedBuilding != null) _siegedBuilding.CheckForSiegingEnemies();

            if (PlayerNumber == 1) ObjectHolder.Instance.buildings.ForEach(x => x.CheckForSiegingEnemies());
        }
        ...
        public virtual void OnUnitSelected()
        {
            ...
            if (UnitSelected != null) { UnitSelected.Invoke(this, new EventArgs()); }
        }
        ...
        {
            SetState(new UnitStateMarkedAsFriendly(this));
            if (UnitDeselected != null) { UnitDeselected.Invoke(this, new EventArgs()); }
        }
        ...
        {
            UnitController u = (UnitController) other;
            if (PlayerNumber == 0 && u.GetBuilding() != null) return false;
        ...
        {
            if (!IsUnitAttackable(unitToAttack, Cell)) { return; }

            AttackAction attackAction = DealDamage(unitToAttack);
            MarkAsAttacking(unitToAttack);
            float damageInflicted = attackAction.Damage;

            if (cell is HexagonTile)
            {
                HexagonTile tile = (HexagonTile) cell;
                if (tile.isMountain)
                {
                    float multiplierAttack = (float) Modifiers.Instance.mountainAttackModifier / 100;
                    damageInflicted += damageInflicted * multiplierAttack;
                }
            }

            unitToAttack.DefendHandler(this, (int) damageInflicted, true, false, "");
            AttackActionPerformed(attackAction.ActionCost);

            if (unitToAttack.unitType == UnitType.Building && unitType != UnitType.Building)
            {
                _siegedBuilding = (unitToAttack as UnitController).GetBuilding();
                _siegedBuilding.CheckForSiegingEnemies();
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        protected virtual AttackAction DealDamage(Unit unitToAttack) { return new AttackAction(AttackFactor, 1f); }
        ...
        public void DefendHandler(Unit aggressor,
                                  int damage,
                                  ...
                                  string extraText)
        {
            ...
            if (ignoreDefense)
                ...
                damageTaken = Defend(aggressor, damage);
            ...
            if (cell is HexagonTile)
            {
                ...
                if (tile.isMountain)
                {
                    float multiplierDefense = (float) Modifiers.Instance.mountainDefenseModifier / 100;
                    damageTaken -= damageTaken * multiplierDefense;
                }
            }

            HitPoints -= (int) damageTaken;
            UpdateHealthbar();
            DefenceActionPerformed();

            if (UnitAttacked != null) { UnitAttacked.Invoke(this, new AttackEventArgs(aggressor, this, damage)); }

            if (HitPoints <= 0)
            {
                if (UnitDestroyed != null) { UnitDestroyed.Invoke(this, new AttackEventArgs(aggressor, this, damage)); }

                if (unitType != UnitType.Building && aggressor != this) AudioController.Instance.SFXWeaponImpact();
            ...
            if (showDamageIndicator)
                Instantiate(ObjectHolder.Instance.unitHealthText, transform.position, Quaternion.Euler(0f, 0f, -90f))
                    .UpdateText((int) -damageTaken, extraText);
        }
        ...
        public virtual void Move(Cell destinationCell, List<Cell> path)
        {
            ...
            if (MovementAnimationSpeed > 0) { StartCoroutine(MovementAnimation(path)); }
            else { transform.position = Cell.transform.position; }

            if (UnitMoved != null) { UnitMoved.Invoke(this, new MovementEventArgs(Cell, destinationCell, path)); }
        }
        ...
        protected virtual IEnumerator MovementAnimation(List<Cell> path)
        {
            ...
            {
                Vector3 destination_pos = new Vector3(cell.transform.localPosition.x, cell.transform.localPosition.y,
                ...
                while (transform.localPosition != destination_pos)
                {
                    if (PlayerNumber != 0)
                    {
                        ...
                        {
                            transform.localPosition = Vector3.MoveTowards(transform.localPosition, destination_pos,
                                                                          Time.deltaTime * 20);
                        }
                        else
                        {
        ...
        /// </summary>
        public virtual bool IsCellMovableTo(Cell cell) { return !cell.IsTaken; }

        /// <summary>
        /// Method indicates if unit is capable of moving through cell given as parameter.
        /// </summary>
        public virtual bool IsCellTraversable(Cell cell) { return !cell.IsTaken; }
        ...
        public HashSet<Cell> GetAvailableDestinations(List<Cell> cells)
        {
            ...
            foreach (var key in paths.Keys)
            {
                ...
                if (pathCost <= MovementPoints) { cachedPaths.Add(key, path); }
            }
        ...
        {
            if (cachedPaths != null && cachedPaths.ContainsKey(destination)) { return cachedPaths[destination]; }
            else { return _fallbackPathfinder.FindPath(GetGraphEdges(cells), Cell, destination); }
        }
        ...
        /// <summary>
        /// Method updated the healthbar sprite Y-size
        /// </summary>
    ...
    public class UnitCreatedEventArgs : EventArgs
    {
        ...
        public UnitCreatedEventArgs(Transform unit) { this.unit = unit; }
    }
}